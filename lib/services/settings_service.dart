...
class SettingsService extends GetxService {
  ...
  void _setThemeMode(ThemeMode mode) {
    ...
    _brightness.value = mode == ThemeMode.dark
        ? Brightness.dark
        : Brightness.light;
  }
  ...
  Future<void> _handleNotificationToggle(
    ...
    try {
      // Check if FCMService is registered, if not wait for it or initialize it
      if (!Get.isRegistered<FCMService>()) {
        debugPrint('FCMService not registered yet, initializing...');
        // Put FCMService synchronously if it doesn't exist
      ...
      debugPrint('Error in notification toggle: $e');
      // Revert UI state if backend operation failed
      _notificationsEnabled.value = !value;
      _showErrorSnackBar(context);
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          label: 'Retry',
          onPressed: () =>
              toggleNotifications(_notificationsEnabled.value, context),
        ),
      ),
    );
  }
  ...
  IconData get notificationIcon {
    return notificationsEnabled ? Icons.notifications : Icons.notifications_off;
  }
}
